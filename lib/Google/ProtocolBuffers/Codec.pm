...
BEGIN {
    ...
    if ($ivsize>=8) {
        require 'Google/ProtocolBuffers/CodecIV64.pm'; ## no critic
    } elsif ($ivsize==4) {
        require 'Google/ProtocolBuffers/CodecIV32.pm'; ## no critic
    } else {
...
sub encode
{
    ...
    foreach my $field (@{ $self->_pb_fields_list }) {
        ...
        $cardinality = LABEL_OPTIONAL unless $cardinality;
