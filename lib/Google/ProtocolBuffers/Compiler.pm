...
syntax      :   "syntax" "=" strLit ## syntax = "proto2" or "proto3";
                {
                    die "Unknown syntax" unless $item{strLit} =~ m!proto2|3!;
                    $return = '';
                }

END_OF_GRAMMAR2

my $grammar3 = <<'END_OF_GRAMMAR3';

proto       :   <skip: qr!  (?: //.*\n | \s+ )*  !x>
                ## list of top level declarations.
                ## Skip empty declarations and ";".
                (message | extend | enum | package | option | service | syntax | ";")(s) /\Z/
                { $return = [ grep {ref $_} @{$item[2]} ]; }
            |   <error>
...
extend      :   "extend" <commit> userType "{" ( field | ";" )(s?) "}"
                { $return = [extend => $item{userType}, [ grep {ref $_} @{$item[5]}] ]}
...
messageBody :   "{" <commit> ( field | enum | message | ";" )(s?) "}"
                { $return = [ grep {ref $_} @{$item[3]} ] }
            |   <error?> <reject>

field       :   type ident "=" intLit fOptList(?) ";"
                { $return = [field => $item{type}, $item{ident}, $item{intLit}, $item{fOptList} ] }
...
service     :   "service" <commit> ident serviceBody
                { $return = [ service => $item{ident}, $item{serviceBody} ]; }
            |   <error?> <reject>

serviceBody :   "{" <commit> ( rpc | option | ";" )(s?) "}"
                { $return = [ grep {ref $_} @{$item[3]} ]; }
            |   <error?><reject>
...
syntax      :   "syntax" "=" strLit ## syntax = "proto3";
                {
                    die "Unknown syntax" unless $item{strLit} eq 'proto3';
                    $return = '';
                }
...
sub parse {
    ...
    while ($text || @import_files)  {
        ...
        my ($grammar_ver) = $content =~ /syntax\s+=\s+"proto(2|3)"\s*;/;
        $self->{'grammar_ver'} = $grammar_ver ? $grammar_ver : 2;
        my $parser = Parse::RecDescent->new($self->{'grammar_ver'} == 2 ? $grammar2 : $grammar3) or die;
...
sub collect_names {
    ...
    foreach my $decl (@$nodes) {
        ...
        if ($kind eq 'package') {
            ...
            $symbol_table->add('service' => $decl->[1], $context);
        } else {
            warn $kind;
        }
    }
    ...
    foreach my $decl (@$nodes) {
        ...
        if ($kind eq 'package') {
            ...
            if ($self->{'grammar_ver'} == 2) {
                ...
                push @$fields_list, [undef, $type_name, $name, $field_number, $default_value];
            }
        } elsif ($kind eq 'enumField') {
            confess unless $destination_type_name;
...
sub _add {
    ...
    ## no fully quilified names are allowed to declare (so far)
    die if $name =~ /\./;
    my $fqn;
    if ($context) {
